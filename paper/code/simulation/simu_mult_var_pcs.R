...
source('/home/kortest/argo_functional_regression/paper/code/src/simulation_src.R')
...
for (m in 3:6){
  ...
  EM_params[['pc_response2']] <- 6
  EM_params[['pc_predictors']] <- m
  
  r= foreach(bs = 1:100, .verbose = T, .errorhandling = 'pass') %dopar% {
  ...
  save(r, file = paste0('~/argo_functional_regression/paper/results/mult_var_simu_pcs',as.character(m),'.RData'))
}
